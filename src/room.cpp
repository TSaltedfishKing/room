...
namespace Room {
    ...
    std::vector<RoomInfo> GetAllRoomInfo() {
        ...
        for (const auto &section: sections) {
            ...
            roomInfo.waitingPosition = StringToVec3(
                    WideStringToString(ini.GetValue(section.pItem, L"waitingPosition")));
    ...
    std::vector<SubRoomInfo> GetAllSubRoomInfo(int room_id) {
        ...
        for (const auto &section: sections) {
            ...
            subroomInfo.aabbPosition = pos2AABB(
                    StringToVec3(WideStringToString(ini.GetValue(section.pItem, L"positionA"))),
                    StringToVec3(WideStringToString(ini.GetValue(section.pItem, L"positionB")))
            );
    ...
    Vec3 GetSubRoomCheckPoint(int ID, int ID_SUB) {
        ...
        if (ini.GetSectionSize(StringToWideString(std::to_string(ID))) != -1) {
            return StringToVec3(
                    WideStringToString(ini.GetValue(StringToWideString(std::to_string(ID_SUB)), L"checkpoint")));
        } else {
            return {0, 0, 0};
        }
    }
    ...
    bool SetRoomInfo(int ID, Vec3 waitingPosition) {
        auto room_id = StringToWideString(std::to_string(ID));
        auto _waitingPosition = StringToWideString(Vec3ToString(waitingPosition));

        CSimpleIniW ini;
        ini.LoadFile(PLUGIN_CONFG_FILE.c_str());
        if (ini.GetSectionSize(StringToWideString(std::to_string(ID))) != -1) {
            ini.SetValue(room_id, L"waitingPosition", _waitingPosition);
            ini.SaveFile(PLUGIN_CONFG_FILE.c_str());
            return true;
        } else {
            return false;
    ...
    bool SetRoomInfo(int ID, const string &description, int maxSubRoomPlayerCount, Vec3 waitingPosition) {
        auto room_id = StringToWideString(std::to_string(ID));
        auto _description = StringToWideString(description);
        auto _maxSubRoomPlayerCount = StringToWideString(std::to_string(maxSubRoomPlayerCount));
        auto _waitingPosition = StringToWideString(Vec3ToString(waitingPosition));
    ...
    bool SetRoomInfo(int ID, const string &description, Vec3 waitingPosition) {
        ...
        if (ini.GetSectionSize(StringToWideString(std::to_string(ID))) != -1) {
            ini.SetValue(room_id, L"description", _description);
            ini.SetValue(room_id, L"waitingPosition", StringToWideString(Vec3ToString(waitingPosition)));
            ini.SaveFile(PLUGIN_CONFG_FILE.c_str());
            return true;
        } else {
            return false;
    ...
    bool SetRoomInfo(int ID, const string &description, std::vector<SubRoomInfo> subroomInfo) {
        auto room_id = StringToWideString(std::to_string(ID));
        auto _description = StringToWideString(description);
    ...
    bool SetSubRoomInfo(int ID, int ID_SUB, Vec3 checkpoint, Vec3 positionA, Vec3 positionB) {
        ...
        if (ini.GetSectionSize(room_id_sub) != -1) {
            ...
            ini.SetValue(room_id_sub, L"positionA", StringToWideString(Vec3ToString(positionA)));
            ini.SetValue(room_id_sub, L"positionB", StringToWideString(Vec3ToString(positionB)));
